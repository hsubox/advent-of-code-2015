...
function applyToRect([x1, y1, x2, y2], func) {
  for (let i = x1; i <= x2; i++) {
    for (let j = y1; j <= y2; j++) {
      func([i, j]);
    }
...
function countLightsOn() {
  return lightGrid.reduce((sum, row) => sum + row.reduce((a, b) => a + b, 0), 0);
}

function displayLightGrid() {
  console.log(lightGrid.map((row) => row.map((x) => (x) ? '#' : ' ').join('')).join('\n'));
}

function followInstructions(instructions, version) {
  instructions.split('\n').forEach((instruction) => {
    const rect = (/(\d+),(\d+) through (\d+),(\d+)/).exec(instruction).slice(1, 5).map(Number);
    let func = null;
    if (instruction.startsWith('turn on')) {
      func = (version == 1) ? turnOnV1 : turnOnV2;
    } else if (instruction.startsWith('turn off')) {
      func = (version == 1) ? turnOffV1 : turnOffV2;
    } else if (instruction.startsWith('toggle')) {
      func = (version == 1) ? toggleV1 : toggleV2;
    } else {
      throw 'unknown instruction!';
    }
    applyToRect(rect, func);
  });
}

// part 1
function turnOnV1([x, y]) {
  lightGrid[x][y] = 1;
}

function turnOffV1([x, y]) {
  lightGrid[x][y] = 0;
}

function toggleV1([x, y]) {
  lightGrid[x][y] = (lightGrid[x][y]) ? 0 : 1;
}

let lightGrid;
lightGrid = Array(1000).fill(0).map((x) => Array(1000).fill(0));
followInstructions('turn on 0,0 through 999,999', 1);
assert.equal(countLightsOn(), 1000000);
followInstructions('toggle 0,0 through 999,0', 1);
assert.equal(countLightsOn(), 1000000 - 1000);
followInstructions('turn off 499,499 through 500,500', 1);
assert.equal(countLightsOn(), 1000000 - 1000 - 4);

lightGrid = Array(1000).fill(0).map((x) => Array(1000).fill(0));
followInstructions(input, 1);
console.log(countLightsOn());

// part 2
function turnOnV2([x, y]) {
  lightGrid[x][y] += 1;
}

function turnOffV2([x, y]) {
  lightGrid[x][y] = Math.max(lightGrid[x][y] - 1, 0);
}

function toggleV2([x, y]) {
  lightGrid[x][y] += 2;
}

lightGrid = Array(1000).fill(0).map((x) => Array(1000).fill(0));
followInstructions('turn on 0,0 through 0,0', 2);
assert.equal(countLightsOn(), 1);
followInstructions('toggle 0,0 through 999,999', 2);
assert.equal(countLightsOn(), 1 + 2000000);

lightGrid = Array(1000).fill(0).map((x) => Array(1000).fill(0));
followInstructions(input, 2);
console.log(countLightsOn());
