...
function getNthCode(initialCode, base, exp, mod) {
  let c = 1;
  let i = 0;
  while (i < exp) {
    i += 1;
  ...
  return (c * initialCode) % mod;
}

assert.equal(getNthCode(initialCode, base, 1, mod), 31916031);
assert.equal(getNthCode(initialCode, base, 2, mod), 18749137);
let exp = (row + col - 2) * (row + col - 1) / 2 + col - 1;
console.log(getNthCode(initialCode, base, exp, mod));
