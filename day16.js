...
function parseMFCSAM(input) {
  let data = {};
  input.split('\n').forEach((row) => {
    [field, amount] = row.split(': ');
    data[field] = Number(amount);
  });
  return data;
}
...
function parseAunts(input) {
  let aunts = {};
  input.split('\n').forEach((row) => {
    const number = Number(/Sue (\d+):/.exec(row)[1]);
    aunts[number] = {};
    const matches = row.match(/(\w+): (\d+)/g).forEach((pair) => {
      const [item, quantity] = /(\w+): (\d+)/.exec(pair).slice(1, 3);
      aunts[number][item] = Number(quantity);
    });
  });
  return aunts;
}

const description = parseMFCSAM(MFCSAM);
const aunts = parseAunts(input);

const possibleAunts = Object.keys(aunts).map(Number).filter((number) => {
  return Object.keys(aunts[number]).every((item) => {
    return description[item] == aunts[number][item];
  });
});
console.log(possibleAunts);

const possibleAuntsWithRange = Object.keys(aunts).map(Number).filter((number) => {
  return Object.keys(aunts[number]).every((item) => {
    if (item == 'cats' || item == 'trees') {
      return aunts[number][item] > description[item];
    }
...
console.log(possibleAuntsWithRange);
